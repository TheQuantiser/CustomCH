...
The combined datacard "htt_mssm.txt" is now transfered to a MSSM model dependent workspace.
    ...
    text2workspace.py -b output/mssm_nomodel/htt_mssm.txt -o output/mssm_nomodel/htt_cmb_mhmodp.root -P HiggsAnalysis.CombinedLimit.MSSMvsSMRun2:MSSM --PO filePrefix=auxiliaries/models/ --PO modelFiles=8TeV,out.mhmodp-8TeV-tanbHigh-nnlo.root,0

This uses a physics model from the combine tool which reads model files from the directory `auxiliaries/models/`, so we need to add this path as well as the model files themselves as additional parameters. The general syntax for the model files themselves is `--PO modelFiles=ERA,FILE,VERSION`. In this example, we choose the mhmod+ model for high values of tanb at 8TeV. For a list of all available options for text2workspace, run `text2workspace.py -h`. For upcoming 13/14TeV runs `VERSION=1` should be set.
